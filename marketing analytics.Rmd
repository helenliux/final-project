...
leg.txt <- levels(ny_hmda$action_taken_name)
...
summarize(TotalActionTaken = n()) %>%
 mutate(PercentageActionTaken = TotalActionTaken /sum(TotalActionTaken)*100) 
ny_hmdaRace_status
ggplot(ny_hmdaRace_status, aes(x = action_taken_name,y = PercentageActionTaken),stat="count")+ geom_bar(stat='identity',colour="red")+labs( title = ' race destribution by loan actions')+ geom_bar(stat='identity',colour="red")+labs( title = ' Race and Loan Status')+facet_wrap(~ applicant_race_name_1 , scales ="free_y")+theme(axis.text.x = element_text(angle = 30, hjust = 1))+geom_text(data=ny_hmdaRace_status, aes(label=paste0(round(PercentageActionTaken,1),"%"),y=PercentageActionTaken+3), size=2)+geom_point(aes(shape =action_taken_name), alpha = 0) + 
scale_shape_manual(name = "Loan_actions", values = x.labels) + 
guides(shape = guide_legend(override.aes = list(size = 2, alpha = 1))) + 
scale_x_discrete(name = "Loan_actions", labels = x.labels)
...
RegressionVec5 = c('+factor(loan_type)','+purchaser_type')
...
group_by(applicant_sex_name,applicant_ethnicity_name)%>%
summarize(income=mean(applicant_income_000s),lien_status=Mode(lien_status))
customerSegments
```
