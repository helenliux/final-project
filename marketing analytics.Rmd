...
modellist<-list()
for (i in 1:64) {
modellist[[i]]<-(lm(as.formula(formulaSet[i]),data=ny_hmApproval))
} 
...
predlist[[i]] <- predict(modellist2[[i]], newdata = test_data)
reslist[[i]] <- data.frame(actual = test_data$Loan_Approval, predlist[[i]])
likelihood[[i]]<- with(reslist[[i]], ifelse(test_data$Loan_Approval== 1, predlist[[i]], 1-predlist[[i]]))
Loglikelihood[[i]]<--sum(log(likelihood[[i]]))
}
...
likelihood[[1]]<- with(reslist[[1]], ifelse(test_data$Loan_Approval== 1, predlist[[1]], 1-predlist[[1]]))
likelihood[[1]]

b<-sum(log(likelihood[[1]]))
b
